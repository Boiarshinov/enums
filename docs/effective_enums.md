...
### Отдача перечислений
Перед вами REST API, который вам нужно поддержать в своем приложении.
Посмотрите на два примера API, описанных в формате Open API, и подумайте, какой из них вам нравится больше.
```yaml
OperationStatus:
  title: Ответ на запрос статуса операции
  type: object
  properties:
    id:
      type: string
...
OperationStatus:
  ...
  properties:
    ...
    status:
      ...
      - NOT_STARTED
```
Не сомневаюсь, что вы выбрали второй вариант.
Ограничение на количество различных значений определенного поля - всегда упрощает разработку.
Это позволяет клиентам заложить различную логику обработки приходящих значений.

Наверняка в вашем приложении есть подобный код:
```java
var response = restTemplate.postForEntity(url, null, ApiResponse.class);
...
```
Все потому что коды состояния http тоже являются примерами перечислений в API.

Класс `HttpStatus` в Spring как раз является перечислением: 
```java
